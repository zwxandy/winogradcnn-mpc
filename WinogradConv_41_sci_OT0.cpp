...
// void Winograd2x2Kernel(int K, int C, int H, int W, cube* filters, cube& image, cube& result) {
void Winograd2x2Kernel(int K, int C, int H, int W, uint64_t* weight, uint64_t* input, uint64_t* output) {
	// defining constants and values that follow directly from
	// https://arxiv.org/abs/1509.09308
	cout << ">>> Winograd Convolution F(2x2, 3x3) kernel... <<<" << endl;
	...
	// convert array to cube format
	cube image = cube(H, W, C);
	for (int i = 0; i < H; i ++) {
		for (int j = 0; j < W; j ++) {
			for (int k = 0; k < C; k ++) {
				cube(i, j, k) = Arr4DIdxRowM(input, 1, H, W, C, 0, i, j, k);
			}
		}
	}
	...
	for (int i = 0; i < K; i++) {
		...
		for (int j = 0; j < 3; j ++) {
			for (int k = 0; k < 3; k ++) {
				for (int l = 0; l < C; l ++) {
					filters[i](j, k, l) = Arr4DIdxRowM(weight, 3, 3, C, K, j, k, l, i);
				}
			}
		}
	}

	cube result = cube(out_H, out_W, K);

	cout << "Number of input tiles: " << P << endl;
	...
	// computes M, an alpha x alpha x K x P matrix (alpha*alpha times MatMul2D)
	// Comm. is in this part
	for (int xi = 0; xi < alpha; xi++) {
		for (int nu = 0; nu < alpha; nu++) {
			// flop: 16 * K * P * (2C - 1)
			// cout << "U[xi][nu]" << U[2][1](0, 1) << endl;
			M[xi][nu] = U[xi][nu] * V[xi][nu]; // here, comm. from MatMul2D protocol!

			// convert U to U_array
			uint64_t* U_array = make_array<uint64_t>(K, C);
			for (int i = 0; i < K; i ++) {
				for (int j = 0; j < C; j ++) {
					Arr2DIdxRowM(U_array, K, C, i, j) = U[xi][nu](i, j);
				}
			}
			// convert V to V_array
			uint64_t* V_array = make_array<uint64_t>(C, P);
			for (int i = 0; i < C; i ++) {
				for (int j = 0; j < P; j ++) {
					Arr2DIdxRowM(V_array, C, P, i, j) = V[xi][nu](i, j);
				}
			}
			// compute M_array with MatMul2D protocol
			uint64_t* M_array = make_array<uint64_t>(K, P);
			MatMul2D(K, C, P, U_array, V_array, M_array, true);

			// convert M_array back to M
			for (int i = 0; i < K; i ++) {
				for (int j = 0; j < P; j ++) {
					M[xi][nu](i, j) = Arr2DIdxRowM(M_array, K, P, i, j);
				}
			}
		}
	}
	...
	// convert output: cube to array format
	for (int i = 0; i < out_H; i ++) {
		for (int j = 0; j < out_W; j ++) {
			for (int k = 0; k < K; k ++) {
				Arr4DIdxRowM(output, 1, out_H, out_W, K, 0, i, j, k) = result(i, j, k);
			}
		}
	}
	cout << ">>> Finish Winograd Convolution F(2x2, 3x3). <<<" << endl;
}
...
int main(int argc, char** argv)
{
	...
	int H = 7;  // height
	int W = 7;  // width
	int C = 160;  // input channels
	int K = 320;  // output channels
	int r = 3;  // kernel size
	int out_H = H - r + 1;  // output height
	int out_W = W - r + 1;  // output width

	// input
	uint64_t* input = make_array<uint64_t>( (int32_t)N,  (int32_t)H,  (int32_t)W,  (int32_t)C);
	uint64_t __tmp_in_tmp0;
	for (uint32_t i0 =  (uint32_t)0; i0 <  (int32_t)N; i0++){
		for (uint32_t i1 =  (uint32_t)0; i1 <  (int32_t)H; i1++){
			for (uint32_t i2 =  (uint32_t)0; i2 <  (int32_t)W; i2++){
				for (uint32_t i3 =  (uint32_t)0; i3 <  (int32_t)C; i3++){
					if ((party == CLIENT)) {
						cin >> __tmp_in_tmp0;
					}
					Arr4DIdxRowM(input, (int32_t)N, (int32_t)H, (int32_t)W, (int32_t)C,i0,i1,i2,i3) = (party == CLIENT) ? __tmp_in_tmp0 : 0;
				}
			}
		}
	}
	...
	// kernel
	uint64_t* weight = make_array<uint64_t>( (int32_t)r,  (int32_t)r,  (int32_t)C,  (int32_t)K);
	uint64_t __tmp_in_tmp1;
	for (uint32_t i0 =  (uint32_t)0; i0 <  (int32_t)r; i0++){
		for (uint32_t i1 =  (uint32_t)0; i1 <  (int32_t)r; i1++){
			for (uint32_t i2 =  (uint32_t)0; i2 <  (int32_t)C; i2++){
				for (uint32_t i3 =  (uint32_t)0; i3 <  (int32_t)K; i3++){
					if ((party == SERVER)) {
						cin >> __tmp_in_tmp1;
					}
					Arr4DIdxRowM(weight, (int32_t)r, (int32_t)r, (int32_t)C, (int32_t)K,i0,i1,i2,i3) = (party == SERVER) ? __tmp_in_tmp1 : 0;
				}
			}
		}
	}
	...
	uint64_t* output = make_array<uint64_t>( (int32_t)N,  (int32_t)out_H,  (int32_t)out_W,  (int32_t)K);
	// original conv
	// Conv2DWrapper( (int32_t)N,  (int32_t)H,  (int32_t)W,  (int32_t)C,  (int32_t)r,  (int32_t)r,  (int32_t)K,  (int32_t)0,  (int32_t)0,  (int32_t)0,  (int32_t)0,  (int32_t)1,  (int32_t)1, input, weight, output);

	// compute winograd conv
	Winograd2x2Kernel(K, C, H, W, weight, input, output);
